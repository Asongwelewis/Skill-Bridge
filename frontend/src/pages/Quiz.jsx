  ...
  useEffect(() => {
    ...
    const load = async (attempt = 0) => {
      try {
        ...
        if (cancelled) return
        // Quiz is generated by Groq ~1–2s after the session ends — retry on 404.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center theme-transition" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
        <div className="glass-panel card-3d px-6 py-5 flex flex-col items-center gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{generating ? 'Generating your quiz…' : 'Loading quiz…'}</p>
        </div>
      </div>
    )
  }
