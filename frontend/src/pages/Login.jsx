    ...
    <div className="min-h-screen flex theme-transition" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 relative overflow-hidden"
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-20 right-16 w-72 h-72 rounded-full animate-float"
            style={{ background: 'radial-gradient(circle, rgba(79,70,229,0.25) 0%, transparent 70%)' }} />
          <div className="absolute bottom-24 left-12 w-52 h-52 rounded-full"
        ...
        <div className="relative">
          ...
          <p className="animate-slide-up delay-200 text-gray-400 leading-relaxed mb-8">
            Your peer learning network is waiting. Sessions, matches, and new skills — all here.
          </p>
          <div className="space-y-3">
            {['Resume where you left off', 'Check pending matches', 'Join your next session'].map((t, i) => (
              <div key={t} className={`flex items-center gap-3 text-gray-300 text-sm animate-slide-left delay-${(i + 3) * 100}`}>
        ...
        <div className="w-full max-w-md animate-scale-in">
          ...
          <button onClick={handleGoogle} disabled={googleLoading}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl text-sm font-medium
              transition-all duration-200 hover:-translate-y-0.5 mb-6 disabled:opacity-50"
            style={{
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm
                ...
                hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed mt-2">
              {loading
